...
struct Node { //Node that will hold the instructions of an Isomorphic tree
	...
	Node() {
		...
		whyCantBeVectorizable = "*failed to get the reason (Isn't it vectorizable?)*";
	}
	...
	int getOverheadCost() {
		return extractOverheadCost + insertOverheadCost;
	}
	...
	void insertInst(Value *it) {
		ScalarInstructions.push_back(it);
	}
	...
	std::vector<Node*> getLastVectorizableNodes(){
		...
		for(unsigned i = 0; i < LeafNodes.size(); i++) {
			...
			if(leafNodeIsPointedBy->canBeVectorizable) {
				...
				if(std::find(lastVectorizableNodes.begin(), lastVectorizableNodes.end(), leafNodeIsPointedBy) == lastVectorizableNodes.end()) {
...
struct MiniSLP : public FunctionPass {
	...
	bool runOnFunction(Function &F) override {
		...
		for (auto bb = F.begin(); bb != F.end(); bb++) { // For each basic block in the function
			errs() << "\n\n<-------> BASIC BLOCK #" << BBCounter << " <------->\n";

			std::vector<StoreInst*> storeInsts;
			for (auto inst = bb->begin(); inst != bb->end(); inst++) { // For each instruction in the basic block
	...
	IsoTree* castToBiggestType(IsoTree* isoTree) {
		CreateExtInsts(isoTree); //We don't change the last nodes (they're are not vectorizable), so create a SExt or ZExt
		std::vector<Instruction*> instToRecreate = FindInstructionsToRecreate(isoTree);
		RecreateInsts(instToRecreate, isoTree->biggestType);
		std::vector<Value*> newSeeds = CreateTruncateInstsAndGetNewSeeds(isoTree); //We don't change the first nodes (they're are not vectorizable), so create a truncate to their original type
	...
	void RecreateCastedTree(IsoTree* isoTree){

	}

	std::vector<IsoTree *> generateIsomorphicTrees(std::vector<StoreInst *>& storeInsts){
		std::vector<IsoTree *> IsomorphicTrees;
		errs() << "\n\n=== Finding isomorphic trees ===";
		errs() << "\nReceived " << storeInsts.size() << " trees.\n\n";

		std::vector<std::vector<Value*> > vectorOfBestStores = findBestCombinationOfStoreInsts(storeInsts);
	...
	//Find the best combination of stores to create a Isomorphic Tree, currently it only analyses pairs of stores
	std::vector<std::vector<Value*> > findBestCombinationOfStoreInsts(std::vector<StoreInst *>& storeInsts) {
		...
		while(!storeInsts.empty()) {
			...
			int bestPairIndex = -1;
			Value *firstStoreInst = storeInsts[0]; //Get the first store from the std::vector
			...
			for (unsigned k = 1; k < storeInsts.size(); k++) {
				...
				int numberOfCommonNodes = getNumberOfCommnNodes({firstStoreInst, kStoreInst});

				if (higherNumberOfCommonNodes == -1 || numberOfCommonNodes > higherNumberOfCommonNodes) {
			...
			if (higherNumberOfCommonNodes > 0) {
				...
				errs() << "\nIsomorphic tree generated by grouping " << bestStores.size() << " trees:\n";
				...
				errs() << "The tree of store \"" << *firstStoreInst << " \" is not isomorphic with any other tree.\n";
			}
			else {
				errs() << "No remaining trees to compare the store \"" << *firstStoreInst << " \"";
			}
	...
	IsoTree* createIsoTree(std::vector<Value *>& instNode, bool isSeed, IsoTree *tree = new IsoTree(), Node *previousNode = nullptr) {
		...
		if (checkIfAllValuesAreInstructions(instNode)) {
			...
			if (checkIfAllInstructionsHaveSameOpCode(instNode)) {
				...
				if(checkIfAllInstructionsAreIntegers(instNode)) {
					...
					if (isa<LoadInst>(instNode[0])) {
						...
						newNode->setAsCantBeVectorizable("it's a node of Load Instructions. (final of the tree)");
					}
					...
					// and need to set up some variables in tree
						...
						newNode->setAsCantBeVectorizable("it's a node of Store Instructions (beggining of the tree).");
					...
					// If it's another instruction, we continue the tree
						...
						unsigned numOps = dyn_cast<Instruction>(instNode[0])->getNumOperands();
				...
				newNode->setAsCantBeVectorizable("not all instructions in the node have the same Opcode.");
			...
			newNode->setAsCantBeVectorizable("not all elements of the node are instructions.");
		}
	...
	bool checkIfAllInstructionsHaveSameOpCode(const std::vector<Value *>& instrucs){
	...
	bool checkIfAllInstructionsHaveTheSameType(const std::vector<Value *>& instrucs){
		if(!instrucs.empty()) {
	...
	int getNumberOfCommnNodes(const std::vector<Value *>& instNode, int commonNodesSoFar = 0) {

		if (checkIfAllValuesAreInstructions(instNode) &&
		    checkIfAllInstructionsHaveSameOpCode(instNode) &&
		    checkIfAllInstructionsAreIntegers(instNode)) {

			// If it's a Load Instrunction, then it's a leaf (right now we will not vectorize loads)
			if (isa<LoadInst>(instNode[0])) {
			}
			...
			// If it's a Store Instrunction, then we need just the ValueOperand
				...
				commonNodesSoFar = getNumberOfCommnNodes(OperandInsts, commonNodesSoFar);
			}
	...
	void CreateExtInsts(IsoTree* isoTree){
		...
		std::vector<Node*> lastVectorizableNodes = isoTree->getLastVectorizableNodes();
		std::vector<Value*> newOperands;

		for(unsigned nodeIndex = 0; nodeIndex < lastVectorizableNodes.size(); nodeIndex++) {  //For every last vectorizable node
			Node* node  = lastVectorizableNodes[nodeIndex];

			for(unsigned scalarIndex = 0; scalarIndex < node->ScalarInstructions.size(); scalarIndex++) {  //We go through the instructions
				Instruction* nodeInst = dyn_cast<Instruction>(node->ScalarInstructions[scalarIndex]);
				IRBuilder<> builder(nodeInst);

				for(unsigned operandIndex = 0; operandIndex < nodeInst->getNumOperands(); operandIndex++) {  //And cast its operands to the biggest type
	...
	// LLVM doesn't differenciate signed integers from unsigned ones, only the instructions
	// that use them, but since we need to create a cast, we need to know if a int is or not signed
	// in order to use SExt or ZExt instructions.
	// Currently it only verifies by looking the users tree of the value and checking if 'udiv' appears.
	...
	std::vector<Instruction*> FindInstructionsToRecreate(IsoTree* isoTree){
		...
		return FindInstructionsToRecreate(isoTree, isoTree->getLastVectorizableNodes(), instToRecreate);
	}

	std::vector<Instruction*> FindInstructionsToRecreate(IsoTree* isoTree, const std::vector<Node*> nodesToBeAnalyzed,
	                                                     std::vector<Instruction*>& instToRecreate){
		for(unsigned i = 0; i < nodesToBeAnalyzed.size(); i++) {
			...
			if(node->canBeVectorizable) {
				for(unsigned k = 0; k < node->ScalarInstructions.size(); k++) {
					instToRecreate.push_back(dyn_cast<Instruction>(node->ScalarInstructions[k]));
				}
				FindInstructionsToRecreate(isoTree, {node->isPointedByNode}, instToRecreate);
			}
		}
	...
	//Currently we are only working with BinaryOps, so we're justing mutating the type
	...
	std::vector<Value*> CreateTruncateInstsAndGetNewSeeds(IsoTree* isoTree){
		...
		for(unsigned i = 0; i < node->ScalarInstructions.size(); i++) {
			...
			if(siOperand == newOperand) { //If the instruction didn't change, it's already a tree of the bigges type
	...
	void showTreeHelper(const std::vector<Value *>& instrucs, int space = 0, std::vector<bool> writePipe = {}) {

		for (unsigned i = 0; i < instrucs.size(); i++) {
			if (i == 0) writePipe.push_back(true);
			...
			if (type->isIntegerTy())
				errs() << " integer";
			errs() << ") ~~>" << *si << " ~~> (" << "Scalar Cost = " << GetScalarCost(si, TTI, TLI, Context) << ")";
	...
	void setTreeCost(IsoTree *tree, Node* node = nullptr, bool firstIteration = true) {
		...
		if (node->canBeVectorizable) {
			bool extractAlreadyCalculated = false; //<- We only calculate the extract overhead once
			bool insertAlreadyCalculated = false; //<- We only calculate the insert overhead once

			for (unsigned i = 0; i < node->ScalarInstructions.size(); i++) { //For each instruction in the node

				Instruction *nodeInstruction = dyn_cast<Instruction>(node->ScalarInstructions[i]);

				node->scalarCost += GetScalarCost(nodeInstruction, TTI, TLI, Context);

				if(!extractAlreadyCalculated) {
					//Look for extract overheads
					for (auto user : node->ScalarInstructions[i]->users()) { // For each user of this instruction

						if (Instruction* userInstruction = dyn_cast<Instruction>(user)) { //Check if it is another instruction

							if (!tree->isInstructionInVectorizableTree(userInstruction)) // And check if this user is in the tree and is vectorizable, if not, we have an extract overhead
							{
								int overhead = GetVectorExtractCost(nodeInstruction, tree->vectorWidth, TTI, TLI, Context);
				...
				if(!insertAlreadyCalculated) {
					...
					for (unsigned k = 0; k < nodeInstruction->getNumOperands(); k++) {
						...
						if (Instruction* opInstruction = dyn_cast<Instruction>(Op)) { //Check if it is another instruction

							if (!tree->isInstructionInVectorizableTree(opInstruction)) {

								int overhead = GetVectorInsertCost(nodeInstruction, tree->vectorWidth, TTI, TLI, Context);
			...
			node->vecCost = GetVectorCost(
				dyn_cast<Instruction>(node->ScalarInstructions[0]),
	...
	void showIsoTree(IsoTree *tree, Node* currentNode = nullptr, int space = 0, std::vector<bool> writePipe = {}) {
		...
		if(width == -1) {
			...
			if (currentNode->canBeVectorizable) {
				...
				if (checkIfAllInstructionsHaveTheSameType(currentNode->ScalarInstructions)) {
					errs() << " - Vectorizable.";
				} else {
					errs() << " - Non-vectorizable, but can be if we cast it to " << *tree->biggestType << ".";
				}
			} else {
			...
			for (unsigned i = 0; i < currentNode->pointsToNodes.size(); i++) {
				...
				writePipe[writePipe.size() - 1] = (i != currentNode->pointsToNodes.size() - 1);
